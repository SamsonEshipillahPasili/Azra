...
function showContributeDialog(currentPerson) {
    swal({
        ...
        text: "Enter your contribution to " + currentPerson,
        type: "input",
...
function openPaymentCycle(){
     swal({
            ...
            showCancelButton: true,
            closeOnConfirm: false,
            inputPlaceholder: "Enter bah"
        }, function (inputValue) {
            if(inputValue){
                if(response === "Ok"){
                swal("Great", "You have created a new payment cycle of Ksh." + inputValue, "success");
                $("#openPaymentBtn").removeClass("btn-outline-primary")
                                     .addClass("btn-outline-danger").text("Close Cycle");
                }else{
